#include<stdio.h>
#include<string.h>
#include<stdlib.h>

struct items{
	char item[20];
	float price;
	int qty;
};

struct orders{
	char customer[50];
	char date[50];
	int numofItems;
	struct items itm[50];
};

void CreateBill(char name[50],char date[30]){
	printf("\n\n");
	printf("\t\t\t\t\tDream Cafe\t\t\t\t\t");
	printf("\n\t\t---------------------------------------------------------------------");
	printf("\nDate: %s",date);
	printf("\nInvoice to : %s",name);
	printf("\n\n\n");
//	printf("-----------------------------------------------------------------------");
	printf("Items\t\t");
	printf("Quantity\t\t");
	printf("Total\t\t");
	printf("\n\t\t---------------------------------------------------------------------");
	printf("\n");

}

void LogicForBill(char item[20],int qty,float price){
	printf("%s\t\t",item);
	printf("   %d\t\t",qty);
	printf("         %.2f\t\t",qty*price);
	printf("\n");
}

void DisplayBill(float total){
	printf("\n");
	float dis = 0.1*total;
	float netTotal = total-dis;
	float gst=0.18*netTotal;
	float grandTotal = netTotal+gst;
	printf("\n------------------------------------------------------------------------");
	printf("\nSub Total\t\t\t ",total);	
    printf("\nDiscount @10%\t\t\t%.2f ",dis);	
    printf("\n\t\t\t\t----------------------------------------------------------------");
	printf("\nNet Total\t\t\t%.2f ",netTotal);	
	printf("\nGST @18% \t\t\t\t%.2f ",gst);	
	printf("\n\t\t\t\t----------------------------------------------------------------");
	printf("\nGrand Total\t\t\t%.2f ",grandTotal);
	printf("\n\t\t\t\t----------------------------------------------------------------");		
	
	
}
int main(){
	int opt,n,j;
	char name[50];
	char saveBill='y',flag='y';
	FILE *fp;
	
	while(flag == 'y'){
		float total;
		int i = 0;
		struct orders ord;
		struct orders order;
		printf("======================================================DREAM CAFE========================================================");
		
		printf("\n1.Generate Invoice");
		printf("\n2.Show all Invoices");
		printf("\n3.Search Invoice");
		printf("\n4.Exit");
	    
	    printf("\n\nYour Choice:");
	    scanf("%d",&opt);
	    
	    printf("\nYour choice is %d",opt);
	    printf("\n");
	    
	    switch(opt){
	    	case 1:
	    		
	    		printf("\nPlease enter the name of Customer:");
	    		fgetc(stdin);
	    		fgets(ord.customer,50,stdin);
	    		ord.customer[strlen(ord.customer)-1]=0;
	    		strcpy(ord.date,__DATE__);
	    		printf("Please enter the number of items:\t");
	    		scanf("%d",&n);
	    		ord.numofItems = n;
	    		
	    		for( i=0;i<n;++i){
	    			fgetc(stdin);
	    			printf("\n\n");
	    			printf("Please enter the item %d:\t",i+1);
	    			fgets(ord.itm[i].item,20,stdin);
	    			ord.itm[i].item[strlen(ord.itm[i].item)-1]='\0';
	    			printf("\nPlease enter the quantity:\t");
	    			scanf("%d",&ord.itm[i].qty);
	    			printf("\nPlease enter the unit price:\t");
	    			scanf("%f",&ord.itm[i].price);
	    			total+=ord.itm[i].qty * ord.itm[i].price;
	    			
				}
				
				CreateBill(ord.customer,ord.date);
				
				for( i=0;i<ord.numofItems;++i){
					LogicForBill(ord.itm[i].item,ord.itm[i].qty,ord.itm[i].price);
				}
				DisplayBill(total);
				printf("\nTo save your invoice [y/n]:\t");
				scanf("%s",&saveBill);
				if(saveBill == 'y'){
					fp = fopen("DreamCafe.txt","a+");
					fwrite(&ord,sizeof(struct orders),1,fp);
					
					if(fwrite != 0){
						printf("\nSaved Successfully\n");
					}else{
						printf("\nError Saving");
					}
					fclose(fp);
				}
				
			break;	
			
			case 2:
			
		    	fp = fopen("DreamCafe.txt","r");
		    	printf("\n******************************Previous Invoices*********************************");
			    while(fread(&order,sizeof(struct orders),1,fp)){
				float tot = 0;
				CreateBill(order.customer,order.date);
				for(i=0;i<order.numofItems;++i){
					LogicForBill(order.itm[i].item,order.itm[i].qty,order.itm[i].price);
					tot+=order.itm[i].qty * order.itm[i].price;
				}
				
				DisplayBill(tot);
			}
			    fclose(fp);
			break;


			case 3:
			
			printf("\nPlease enter the name of Customer:");
	    	fgetc(stdin);
	    	fgets(name,50,stdin);
	    	name[strlen(name)-1]='\0';
			fp = fopen("DreamCafe.txt","r");
			printf("\n******************************Invoices of %s*********************************");
			
			while(fread(&order,sizeof(struct orders),1,fp)){
				float tot = 0;
				if(!strcmp(order.customer,name)){
					
				
				CreateBill(order.customer,order.date);
				for(j=0;i<order.numofItems;++j){
					LogicForBill(order.itm[i].item,order.itm[i].qty,order.itm[i].price);
					tot+=order.itm[i].qty * order.itm[i].price;
				}
				
				DisplayBill(tot);
				i = 1;
			}
				fclose(fp);
			}
			if(!i){
				printf("No invoice with such name");
			}
			fclose(fp);
		
			break;
			
		case 4:
		
		printf("\n\t THank You for Visting us");
		exit(0);
		break;
		
		default	:
			printf("\n Please enter valid option");
			break;
		}
		
     	printf("\nAnything else ? [y/n]\t");	
     	scanf("%s",&flag);
			
	}
	printf("\n\n");
	printf("\n\t THank You for Visting us");
	return 0;
}
